...
if __name__ == '__main__':
    ...
    parser.add_argument("--data_root", type=str, default='/cis/home/knaraya4/data/ijbs_aligned_112')
    parser.add_argument('--model', type=str, default='r50')
    parser.add_argument('--model_name', type=str, default='ijcb_cosface')
    ...
    # load model
    model_load_path = os.path.join("/cis/home/knaraya4/arcface_torch/moe", args.model_name, "model.pt")
    model = load_pretrained_model(args.model, model_load_path, args.gpu, args.num_experts, args.k)
    model.to('cuda:{}'.format(args.gpu))

    # make result save root
    save_root = get_save_path(model_load_path)
    os.makedirs(save_root, exist_ok=True)
    if args.data_root == '/cis/home/knaraya4/data/ijbs_aligned_112':
        image_path_df = pd.read_csv('/cis/home/knaraya4/data/IJBS/image_paths_112_new.csv', index_col=0)
    elif args.data_root == '/mnt/store/knaraya4/data/ijbs_aligned_180':
        image_path_df = pd.read_csv('/cis/home/knaraya4/data/IJBS/image_paths_180.csv', index_col=0)
    ...
    for partition_idx in tqdm(range(num_partition)):
        ...
        dataloader = data_utils.prepare_imagelist_dataloader(image_paths, batch_size=args.batch_size, image_size=args.image_size, num_workers=8, image_is_saved_with_swapped_B_and_R=True)
        ...
        #### Resume ###
        # features = np.load(os.path.join(save_root, 'feature_extracted/ijbs_pred_{}_{}.npy'.format(args.model_name, partition_idx)))
        # norms = np.load(os.path.join(save_root, 'feature_extracted/ijbs_pred_{}_norm_{}.npy'.format(args.model_name, partition_idx)))
