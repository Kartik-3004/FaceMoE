...
class ListDatasetWithIndex(Dataset):
    def __init__(self, img_list, image_size, image_is_saved_with_swapped_B_and_R):
...
class ListDataset(Dataset):
    def __init__(self, img_list, image_size, image_is_saved_with_swapped_B_and_R=False):
...
def prepare_imagelist_dataloader(img_list, batch_size, image_size, num_workers=16, image_is_saved_with_swapped_B_and_R=False):
    # image_is_saved_with_swapped_B_and_R: correctly saved image should have this set to False
    # face_emore/img has images saved with B and G (of RGB) swapped. 
    # Since training data loader uses PIL (results in RGB) to read image 
    # and validation data loader uses cv2 (results in BGR) to read image, this swap was okay.
...
def prepare_dataloader(img_list, batch_size, image_size, num_workers=16, image_is_saved_with_swapped_B_and_R=False):
    # image_is_saved_with_swapped_B_and_R: correctly saved image should have this set to False
    # face_emore/img has images saved with B and G (of RGB) swapped.
    # Since training data loader uses PIL (results in RGB) to read image
    # and validation data loader uses cv2 (results in BGR) to read image, this swap was okay.
