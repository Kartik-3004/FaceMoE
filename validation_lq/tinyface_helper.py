...
class TinyFaceTest:
    def __init__(self, tinyface_root='/mnt/store/knaraya4/data', alignment_dir_name='aligned_pad_0.1_pad_high'):
    ...
    def test_identification(self, features, ranks=[1,5,20], save_failures=False):
        ...
        # Pass the save_failures flag to DIR_FAR
        ret_val = DIR_FAR(score_mat, label_mat, ranks, get_false_indices=save_failures)

        if save_failures:
            results, FARs, thresholds, match_indices, false_retrieval, false_reject, false_accept, sort_idx_mat_m = ret_val
            
            # Logic to extract filenames of Rank-1 failures
            # false_retrieval shape: [num_FARs, num_ranks, num_valid_probes]
            # We assume index 0 for FAR (Closed set) and index 0 for Rank (Rank-1)
            rank_1_failures_mask = false_retrieval[0, 0, :]
            
            # match_indices tells us which probes actually had a mate in the gallery
            valid_probe_paths = self.probe_paths[match_indices]
            
            # Filter the valid probes by the failure mask
            failed_probe_paths = valid_probe_paths[rank_1_failures_mask]
            
            # Return results and the list of failed paths
            return results, failed_probe_paths
        else:
            # Original behavior
            results, _, __ = ret_val
            return results
